...
if (FALSE) {
  ...
  outcome_file    <- "/Users/xx20081/Downloads/GCST90302000.tsv"
...
# Write fail rows for both steiger values (used on early exits before the loop)
write_fail_both <- function(dt, file) {
  fwrite(rbindlist(list(
    copy(dt)[, steiger_filtering := FALSE],
    copy(dt)[, steiger_filtering := TRUE]
  )), file, sep = "\t")
}
...
if (nrow(out) == 0) {
  ...
  write_fail_both(fail_dt, results_file)
...
# ── 7. STEIGER r² COMPUTATION (once, shared across both loop iterations) ──────
...
har_dat_base <- har_dat   # save pre-filter copy; reused each loop iteration


# ── 8-9. LOOP OVER STEIGER FILTER VALUES ─────────────────────────────────────
results_list <- vector("list", 2L)

for (si in 1:2) {
  steiger_filter <- c(FALSE, TRUE)[si]
  cat("\n--- Steiger filter:", steiger_filter, "---\n")

  fail_dt_i <- copy(fail_dt)
  fail_dt_i[, steiger_filtering := steiger_filter]

  har_dat <- har_dat_base

  # ── Apply Steiger filter ──────────────────────────────────────────────────
  ...
  fail_dt_i[, nsnp_steiger_filtered := nsnp_steiger_filtered]
  fail_dt_i[, snps_steiger_filtered := snps_steiger_filtered]

  if (nrow(har_dat) == 0) {
    cat("WARNING: No SNPs retained after Steiger filtering for", exp_id, "->", out_id, "\n")
    results_list[[si]] <- fail_dt_i
    next
  }
  ...
  presso_msg <- NA_character_
  res_presso <- withCallingHandlers(
    tryCatch(
      TwoSampleMR::run_mr_presso(har_dat, NbDistribution = 1000, SignifThreshold = 0.05)[[1]],
      error = function(e) { presso_msg <<- conditionMessage(e); NULL }
    ),
    warning = function(w) {
      presso_msg <<- if (is.na(presso_msg)) conditionMessage(w) else paste(presso_msg, conditionMessage(w), sep = "; ")
  ...
  presso_dt <- if (!is.null(res_presso)) {
    tryCatch({
      ...
      rbind(
        data.table(
          ...
          nsnp            = sum(har_dat$mr_keep, na.rm = TRUE),
          ...
          presso_error    = presso_msg
        ),
        data.table(
          method          = "MR-PRESSO-corrected",
          nsnp            = sum(har_dat$mr_keep, na.rm = TRUE) - n_outliers,
      ...
      data.table(
        ...
        presso_error = c(conditionMessage(e), NA_character_)
      )
    })
  } else {
    data.table(
      method       = c("MR-PRESSO-raw", "MR-PRESSO-corrected"),
      presso_error = c(presso_msg, NA_character_)
    )
  }
  ...
  results_list[[si]] <- res_main
}

cat("Writing:", results_file, "\n")
fwrite(rbindlist(results_list, fill = TRUE), results_file, sep = "\t")
