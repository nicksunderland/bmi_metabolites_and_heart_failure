#### DESCRIPTION #######################################
# Purpose of script:
# Runs the individual metabolite MR analyses
#
# Author: Nick Sunderland
#
# Date Created: 2025-07-31
#
# Email: nicholas.sunderland@bristol.ac.uk

# #### SET INPUT #########################################
metab_outcome_mr_results <- snakemake@input[["metab_outcome_mr_results"]]
metab_outcome_pathway_mr <- snakemake@input[["metab_outcome_pathway_mr"]]
asp_hf_mr_plot     <- snakemake@output[["asp_hf_mr_plot"]]
asp_hf_mr_all_plot <- snakemake@output[["asp_hf_mr_all_plot"]]
# ########################################################

if (FALSE) {
  metab_outcome_mr_results = file.path(Sys.getenv("HF_METABOLITE_REPO2"), "output", "tables", "mr_results", "metabolite_outcome_mr_results.tsv.gz")
  metab_outcome_pathway_mr = file.path(Sys.getenv("HF_METABOLITE_REPO2"), "output", "tables", "mr_results", "metabolite_pathway_outcome_mr_results.tsv.gz")
  asp_hf_mr_plot           <- file.path(Sys.getenv("HF_METABOLITE_REPO2"), "output", "figures", "cis_mr", "asp_hf_mr_plot.png")
  asp_hf_mr_all_plot       <- file.path(Sys.getenv("HF_METABOLITE_REPO2"), "output", "figures", "cis_mr", "asp_hf_mr_all_plot.png")

}

# requirements
library(data.table)
library(ggplot2)


# read
outcome_dat <- fread(metab_outcome_mr_results)
pathway_dat <- fread(metab_outcome_pathway_mr)


# process
asp_hf_res <- rbind(
  outcome_dat[exposure == "Asparagine levels" & outcome == "hfpef"],
  pathway_dat[exposure == "aspg_asparagine" & outcome == "hfpef"],
  fill = TRUE
)

p <- ggplot(asp_hf_res, aes(x = b, y = exposure, color = method)) +
  geom_vline(xintercept = 0, color="grey50", linetype="dashed") +
  geom_errorbarh(aes(xmin = b-1.96*b_se, xmax = b+1.96*b_se, group=method), height=0, position = position_dodge(width=.8)) +
  geom_point(position = position_dodge(width=.8), size = 2, fill = "white", stroke = 1.1) +
  scale_color_brewer(palette = "Set1", labels = ~ gsub("_", " ", .x)) +
  # scale_shape_manual(values = c("P \u2265 0.05"=21, "P < 0.05"=19)) +
  theme_light() +
  theme(
    panel.grid.major.y = element_blank(),
    legend.title = element_blank(),
    legend.position = "right",
    axis.title.y = element_blank(),
    axis.title.x = element_text(size = 9),
    axis.text.x = element_text(size = 9),
  ) +
  labs(x = "MR estimate - OR outcome per SD increase in exposure") +
  facet_wrap(~outcome, ncol=1, scales="free_y", labeller = labeller(outcome = function(x) {
    fcase(as.character(x)=="HFpEF", "Non-ischaemic HFpEF",
          as.character(x)=="HFrEF", "Non-ischaemic HFrEF",
          as.character(x)=="All-cause HF", "All-cause HF")
  }))
p

ggsave(asp_hf_mr_plot, p, width = 8, height = 5, dpi=300, bg="white")


bcaa <- c("Valine levels", "Isoleucine levels", "Leucine levels",
          "3-methyl-2-oxobutyrate levels",
          "3-methyl-2-oxovalerate levels",
          "4-methyl-2-oxopentanoate levels")

bcaa_dat <- rbind(
  outcome_dat[grepl(paste0(bcaa, collapse="|"), exposure) & outcome == "hfpef" & !is.na(n_snp)],
  fill = TRUE
)
bcaa_dat[, y_label := paste0(exposure, " (nsnp = ", n_snp, ")")]

# all MR methods
p <- ggplot(bcaa_dat, aes(x = b, y = y_label, color = method)) +
  geom_vline(xintercept = 0, color="grey50", linetype="dashed") +
  geom_errorbarh(aes(xmin = b-1.96*b_se, xmax = b+1.96*b_se, group=method), height=0, position = position_dodge(width=.8)) +
  geom_point(position = position_dodge(width=.8), size = 2, fill = "white", stroke = 1.1) +
  scale_color_brewer(palette = "Set1", labels = ~ gsub("_", " ", .x)) +
  scale_shape_manual(values = c("P \u2265 0.05"=21, "P < 0.05"=19)) +
  theme_light() +
  theme(
    panel.grid.major.y = element_blank(),
    legend.title = element_blank(),
    legend.position = "right",
    axis.title.y = element_blank(),
    axis.title.x = element_text(size = 9),
    axis.text.x = element_text(size = 9),
  ) +
  labs(x = "MR estimate - OR outcome per SD increase in exposure") +
  facet_wrap(~outcome, nrow=1, scales="free_y", labeller = labeller(outcome = function(x) {
    fcase(as.character(x)=="hfpef", "Non-ischaemic HFpEF")
    }))
p

fp <- file.path(Sys.getenv("HF_METABOLITE_REPO2"), "output", "figures", "cis_mr", "bcaa_hf_mr_plot.png")
ggsave(fp, p, width = 8, height = 8, dpi=300, bg="white")







bcka_dat <- rbind(
  pathway_dat[grepl("ppm1k", exposure) & outcome == "hfpef" & !is.na(n_snp)],
  fill = TRUE
)
bcka_dat[, exposure := factor(exposure, levels = rev(paste0("ppm1k_", c("valine","leucine","isoleucine","kic","kiv","kmv"))))]
bcka_dat[, y_label := factor(
  paste0(as.character(exposure), " (nsnp = ", n_snp, ")"),
  levels = paste0(levels(exposure), " (nsnp = ", n_snp[match(levels(exposure), exposure)], ")")
)]


# all MR methods
p <- ggplot(bcka_dat, aes(x = b, y = y_label, color = method)) +
  geom_vline(xintercept = 0, color="grey50", linetype="dashed") +
  geom_errorbarh(aes(xmin = b-1.96*b_se, xmax = b+1.96*b_se, group=method), height=0, position = position_dodge(width=.8)) +
  geom_point(position = position_dodge(width=.8), size = 2, fill = "white", stroke = 1.1) +
  scale_color_brewer(palette = "Set1", labels = ~ gsub("_", " ", .x)) +
  scale_shape_manual(values = c("P \u2265 0.05"=21, "P < 0.05"=19)) +
  theme_light() +
  theme(
    panel.grid.major.y = element_blank(),
    legend.title = element_blank(),
    legend.position = "right",
    axis.title.y = element_blank(),
    axis.title.x = element_text(size = 9),
    axis.text.x = element_text(size = 9),
  ) +
  labs(x = "MR estimate - OR outcome per SD increase in exposure") +
  facet_wrap(~outcome, nrow=1, scales="free_y", labeller = labeller(outcome = function(x) {
    fcase(as.character(x)=="hfpef", "Non-ischaemic HFpEF")
  }))
p

fp <- file.path(Sys.getenv("HF_METABOLITE_REPO2"), "output", "figures", "cis_mr", "bcaa_cis_hf_mr_plot.png")
ggsave(fp, p, width = 8, height = 6, dpi=300, bg="white")

